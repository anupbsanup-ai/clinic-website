...
const DOCTORS = [
  { id: "1", name: "Dr. P Venkat Rao", specialty: "General Physician", qualifications: "MBBS", photo: "https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?w=400&q=80", fee: 300, experience: 15 },
];

const TIME_SLOTS = [
  "9:00 AM","9:30 AM","10:00 AM","10:30 AM","11:00 AM","11:30 AM",
  "12:00 PM","4:00 PM","4:30 PM","5:00 PM","5:30 PM","6:00 PM","6:30 PM",
];
const BOOKED = ["9:00 AM","10:30 AM","4:00 PM"];
...
export default function BookPage() {
  ...
  function pickDoctor(doc: typeof DOCTORS[0]) {
    setDoctor(doc);
    setTime("");
    setStep("slot");
  }
  ...
  const steps = ["doctor","slot","confirm","success"];
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-[#0a1f44] to-blue-900 py-16 px-6 text-center text-white">
        ...
        <div className="flex flex-wrap gap-3 justify-center mt-6">
          {["Instant WhatsApp confirmation","No queue","Free cancellation","3 expert doctors"].map(t => (
        ...
        <div className="max-w-3xl mx-auto px-6 py-4 flex items-center gap-3">
              ...
              <div key={s.key} className="flex items-center gap-2">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all cursor-pointer ${
                    done ? "bg-teal-500 text-white" : active ? "text-white" : "bg-gray-100 text-gray-400"
                  }`}
                  style={active ? { background: "linear-gradient(135deg,#0d9488,#0ea5e9)" } : {}}
                  onClick={() => {
                    if (done) {
                      if (i === 0) setStep("doctor");
                      if (i === 1) setStep("slot");
                    }
                  }}
                >
          ...
          {doctor && step !== "doctor" && (
            <div className="ml-auto flex items-center gap-2 bg-teal-50 px-3 py-1.5 rounded-full">
              <Image src={doctor.photo} alt={doctor.name} width={20} height={20} className="rounded-full object-cover" />
          ...
          <AnimatePresence mode="wait">
            {step === "doctor" && (
              <motion.div key="doctor" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                  {DOCTORS.map((doc) => (
                    <motion.div key={doc.id} whileHover={{ y: -6, scale: 1.02 }} whileTap={{ scale: 0.97 }}
            ...
            {step === "slot" && doctor && (
              <motion.div key="slot" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}>
                  ...
                  {DATES.map((d) => {
                    ...
                    return (
                      <button key={d} onClick={() => setDate(d)}
                        className={`flex-shrink-0 w-16 py-3 rounded-2xl text-center transition-all font-medium ${
                          sel ? "text-white shadow-lg scale-105" : "bg-white border border-gray-200 text-gray-600 hover:border-teal-400"
                        }`}
                ...
                <p className="text-gray-400 text-xs mt-4 text-center">Greyed out slots are fully booked · Tap any white slot to proceed</p>
              </motion.div>
            )}
            ...
            {step === "confirm" && doctor && (
              <motion.div key="confirm" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}>
                ...
                <div className="bg-gradient-to-r from-teal-50 to-blue-50 border border-teal-100 rounded-2xl p-5 mb-6 flex items-center gap-4">
                  <Image src={doctor.photo} alt={doctor.name} width={64} height={64} className="rounded-xl object-cover flex-shrink-0" />
                ...
                <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8">
                  <div className="space-y-4 mb-6">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-800 focus:outline-none focus:ring-2 focus:ring-teal-400 text-base transition"
                      />
                    </div>
                    <div>
                  ...
                  <button
                    onClick={confirm}
                    className="w-full py-4 rounded-xl font-extrabold text-white text-lg flex items-center justify-center gap-3 transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg"
                  ...
                  <p className="text-gray-400 text-xs text-center mt-3">WhatsApp opens with your booking details pre-filled. Doctor confirms within 5 min.</p>
                </div>
              </motion.div>
            )}
            ...
            {step === "success" && doctor && (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <button onClick={reset}
                  className="btn-brand text-white font-bold px-10 py-3.5 rounded-xl shadow-lg">
